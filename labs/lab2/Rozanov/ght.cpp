...
int main(int argc, char* argv[])
{
  ...
  {
	  KeyPoint point1 = keypoints1[matches[matchInd].queryIdx];
	  KeyPoint point2 = keypoints2[matches[matchInd].trainIdx];
	  Point2f difference = (Point2f)center - point1.pt;

	  float distance = (float)norm(difference);
	  float angle = atan(difference.y / difference.x);

	  float deltaScale = point2.size / point1.size;
	  float deltaAngle = (point2.angle - point1.angle) * (float)CV_PI / 180.0f;

	  float sceneDist = distance * deltaScale;
	  float sceneAngle = angle + deltaAngle;
	  
	  float invSpliteSet = 1.0f / splitSet;
	  Point newCenter = Point(cvRound(point2.pt.x + cos(sceneAngle) * sceneDist) * invSpliteSet,
		  cvRound(point2.pt.y + sin(sceneAngle) * sceneDist) * invSpliteSet);
	  hgt.at<int>(newCenter.y, newCenter.x)++;
  }
  Point maxP;
  ...
  vector<Point2f> kp1(matches.size());
  vector<Point2f> kp2(matches.size());

  for (size_t match = 0; match < matches.size(); match++)
  {
	  kp1[match] = keypoints1[matches[match].queryIdx].pt;
	  kp2[match] = keypoints2[matches[match].trainIdx].pt;
  }
  Mat hommat = findHomography(kp1, kp2, CV_RANSAC);  
